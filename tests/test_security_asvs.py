...
def test_customer_cannot_access_other_customer_reports(client, app):
    ...
    with app.app_context():
        ...
        conn.execute("INSERT OR IGNORE INTO users (username, password_hash, role, customer_id) VALUES ('cust_a', 'scrypt:32768:8:1$1uR9x$a', 'user', 'CUST_A')")
        conn.execute("INSERT OR IGNORE INTO users (username, password_hash, role, customer_id) VALUES ('cust_b', 'scrypt:32768:8:1$1uR9x$a', 'user', 'CUST_B')")
    ...
    client.post('/login', data={'username': 'cust_a', 'password': 'admin123'})
...
def test_customer_cannot_access_admin_routes(client, app):
    ...
    with app.app_context():
        from app.database import get_connection
        conn = get_connection(app.config['DATABASE_PATH'])
        conn.execute("INSERT OR IGNORE INTO users (username, password_hash, role, customer_id) VALUES ('standard_user', 'scrypt:32768:8:1$1uR9x$a', 'user', 'CUST_A')")
        conn.commit()
        
    client.post('/login', data={'username': 'standard_user', 'password': 'admin123'})
...
def test_path_traversal_blocked(client, app):
    ...
    with app.app_context():
        from app.database import get_connection
        conn = get_connection(app.config['DATABASE_PATH'])
        conn.execute("INSERT OR IGNORE INTO users (username, password_hash, role) VALUES ('hacker', 'scrypt:32768:8:1$1uR9x$a', 'admin')")
    ...
    client.post('/login', data={'username': 'hacker', 'password': 'admin123'})
