...
def manage_users():
    ...
    users = g.db.execute(
        "SELECT * FROM users WHERE customer_id = ? ORDER BY created_at DESC", (customer_id,)
    ).fetchall()
...
def update_user_recipe_permissions():
    ...
    allowed_db = [
        ...
        for r in g.db.execute(
            "SELECT recipe_name FROM customer_recipes WHERE customer_id = ?", (current_user.customer_id,)
        ).fetchall()
    ]
